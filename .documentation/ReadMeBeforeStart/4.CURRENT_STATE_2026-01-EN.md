...
**Document Version**: 2026-01-28 (Updated: Electricity Implementation Guide Reference)  
...
| Natural Gas Dashboard | ✅ Complete | 100% | JSON File |
...
- **Database**: SQL Server Express (EcoSphereData - complete data 2019-2025)
...
```
Server: .\SQLEXPRESS
Database: EcoSphereData (upgraded from TestSlimDB)
Authentication: Windows Authentication
Data Coverage: 2019-2025 (complete dataset)
```

**Note**: The database was upgraded from TestSlimDB to EcoSphereData to include complete historical data from 2019 to 2025. All modules now use this unified database.
...
#### Water Module Tables
| Table Name | Purpose | Data Range | Interval | Duration |
|------------|---------|------------|----------|----------|
| TL93 | Rainwater Level (%) | 2018-10-13 to 2025-12-31 | 10 minutes | 7.22 years |
| TL210 | Hot Water Consumption (L/h) | 2018-12-14 to 2025-12-31 | 1 minute | 7.05 years |

**Note**: Water module now uses raw database intervals (10-minute for rainwater, 1-minute for hot water) without aggregation. Both tables have **complete 2-year cycle data** (7+ years total), meeting Tier 1 forecast requirements.
...
- **Detailed Guide**: `.documentation/ReadMeBeforeStart/ELECTRICITY_IMPLEMENTATION_GUIDE.md`
  - Chart.js configurations (zoom, pan, tooltips)
  - Forecast system structure and algorithms
  - Data processing patterns (sign handling, date handling, aggregation)
  - UI components and reusable patterns
  - Performance optimization strategies
  - Common issues and solutions
  - Adaptation guidelines for Water/Thermal dashboards
...
- **Overview Sub-tab**:
  ...
  - Historical data visualization (10-minute intervals)
  - Key metrics: Current, Average, Peak, Min
  - Data source: TL93 (2018-10-13 to 2025-12-31, 7.22 years)
...
**⚠️ CRITICAL: Data Interpretation**

Hot water data (TL210) represents **instantaneous flow rate** (L/h), NOT cumulative consumption:
- Database value: Flow rate in L/h (e.g., 4066 L/h)
- Actual consumption per minute: Flow rate × (1/60) = L/min
- Example: 4066 L/h → 67.77 L/min → If sustained for 5 minutes = 338.85 L total

**Overview Sub-tab** displays TWO charts:

1. **Flow Rate Trend Chart**:
   - Y-axis: L/h (instantaneous flow rate)
   - Data: Raw 1-minute interval readings from TL210
   - Metrics: Average, Peak, Minimum (NO Total - flow rate sum is meaningless)
   - Component: `FlowRateMetricsCards` (custom 3-column layout)
   - Purpose: Show flow rate patterns and peaks

2. **Consumption Trend Chart (Per Minute)**:
   - Y-axis: L/min (actual consumption per minute)
   - Calculation: Flow rate (L/h) × (1/60) = L/min
   - Metrics: Total, Average, Peak, Minimum
   - Purpose: Show actual water consumption amounts
   - Example daily total: 1,700-5,000 L/day (varies by usage patterns)

**Typical Usage Patterns**:
- Baseline flow: ~200 L/h (continuous, may include circulation system)
- Peak flow: ~4,000 L/h (during high usage periods like morning/evening)
- Daily consumption: 1,700-5,000 L (3-layer office building)
- High peaks are brief (5-10 minutes), not sustained

**Forecast Sub-tab**:
- Four-tier algorithm system (same as electricity consumption)
- Input data: Hourly consumption totals (converted from flow rate)
- Conversion: Sum of (flow rate × 1/60) for each minute in the hour
- Forecast periods: 7/14/30 days
- Output: Daily consumption totals (L/day)
- Algorithm selection based on data completeness (2 years = Tier 1)
...
- Frontend Components: `FlowRateMetricsCards.jsx` (custom metrics for flow rate)
...
- Backend Functions: `convertFlowRateToHourlyConsumption()` in waterController.js

**Implementation Notes**:
1. **DO NOT** sum flow rates - they are instantaneous measurements
2. **ALWAYS** convert flow rate to consumption before aggregation
3. **Forecast** uses consumption data, not flow rate data
4. **Baseline flow** (~200 L/h continuous) may indicate circulation system or sensor baseline
5. **Data validation**: 7+ years of data ensures Tier 1 forecast (90% confidence)

---

### 4.5 Natural Gas Dashboard (Added 2026-01-16)

**Data Source**: JSON file (`naturalGasMonthlyUsage.json`) - Monthly readings from 2023-01 to 2025-11

**Two Main Tabs**:

#### 1. Consumption Tab
- Monthly usage chart (bar chart)
- Key metrics: Total, Average, Peak, Min
- Time presets:
  - All Years: 2023-2025 (all available data)
  - Last Year: 2025 (12 months)
  - This Year: 2026 (no data - shows "No data available")
- Data filtering: Strict year-based filtering ensures correct data display
- Unit: GJ (Gigajoules)

#### 2. Forecast Tab
- **Three-tier algorithm system** (optimized for monthly data):
  - Tier 1: Seasonal Weighted Prediction (75% confidence)
    - Formula: `0.4 × LastYear + 0.4 × TwoYearsAgo + 0.2 × Recent3MonthAvg`
    - Requirements: 24 months (2 years) of data
  - Tier 2: Trend-Based Prediction (60% confidence)
    - Linear regression on last 6 months
    - Requirements: 6 months of data
  - Tier 3: Moving Average (45% confidence)
    - Simple average of last 3 months
    - Requirements: 3 months of data
- Forecast periods: 3/6/12 months
- Algorithm selection visualization
- Data availability analysis
- Confidence level indicators

**Data Structure**:
- JSON format with correct month-to-usage mapping
- Each entry: `{year, month, monthKey, monthLabel, usage, readingDate}`
- Reading date represents PREVIOUS month's usage
  - Example: `readingDate: "2025-12-06"` contains November 2025 usage

**Date Handling**:
- Frontend calculates next month after last data point as forecast start
- Backend uses pure math for month calculations (avoids Date object timezone issues)
- Forecast correctly starts from December 2025 (month after last data: November 2025)

**Key Files**:
- Frontend: `NaturalGasPage.jsx`, `ConsumptionTab.jsx`, `ForecastTab.jsx`, `MonthlyUsageChart.jsx`
- Backend: `naturalGasRoutes.js`, `naturalGasController.js`
- Services: `naturalGasService.js`, `naturalGasForecastService.js`, `NaturalGasService.js` (frontend)
- Data: `naturalGasMonthlyUsage.json` (converted from CSV)
- Constants: `naturalGasConstants.js` (backend), `naturalGas.js` (frontend)
- Scripts: `convert-natural-gas-csv-to-json.js` (data conversion utility)

**Code Quality**:
- ✅ Follows backend standards (asyncHandler, sendSuccess/sendError, validateDateRange)
- ✅ Follows frontend standards (component size < 300 lines, Hooks at top, custom Hook pattern)
- ✅ No magic numbers/strings (centralized constants)
- ✅ Clean separation of concerns (Routes → Controllers → Services)
- ✅ Defensive programming (input validation, error handling)

---

### 4.6 Thermal Dashboard
...
# Natural Gas Data
GET /api/natural-gas/consumption?dateFrom=2023-01-01&dateTo=2025-12-31
GET /api/natural-gas/date-range
GET /api/natural-gas/all-data
GET /api/natural-gas/forecast?targetDate=2025-12-01&forecastMonths=6
