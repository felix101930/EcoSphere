...
| P2       | Forecast/Prediction       | Skeleton only         | MOSTLY IMPLEMENTED                 | 90%        | Electricity + Water + Thermal forecasts |
...
| Electricity      | SQL Server     | SQL Server                   | ✅ COMPLETED     | Using optimized hourly aggregation        |
| Water            | SQL Server     | SQL Server                   | ✅ COMPLETED     | TL93 (rainwater), TL210 (hot water)       |
| Carbon Footprint | SQL Server     | SQL Server + Electricity Maps API | ✅ COMPLETED | TL341 consumption + historical intensity  |
| Users            | SQL Server     | Mock JSON (users.json)       | 📋 PLANNED       | 3 test users                              |
| Login Logs       | SQL Server     | Mock JSON (loginLogs.json)   | 📋 PLANNED       | Tracks login/logout                       |
...
- ✅ USABLE - Primary (Long-term Data): 3 tables (TL341, TL340, TL339)
- ⚠️ USABLE - Secondary (Limited/Cumulative): 15 tables
- 📊 USABLE - Historical Only: 1 table (TL3 - replaced by TL340)
- ❌ UNUSABLE - Invalid Data: 2 tables (TL208, TL388)

**Key Findings**:

1. **Primary Data Sources (594 days: 2019-03-25 → 2020-11-08)**:
...
**Next Steps**: 
- ✅ COMPLETED: Electricity Dashboard implemented with SQL Server integration
...
- **Net Energy Tab**:
  ...
  - **Self-Sufficiency Rate** (COMPLETED 2026-01-07):
    - Average self-sufficiency rate metric card showing (Generation / Consumption) × 100%
    - Combined chart: Net Energy & Self-Sufficiency Rate Trend (dual Y-axis)
    - Net Energy: Red/Green line (negative = deficit, positive = surplus)
    - Self-Sufficiency Rate: Orange line with 100% threshold reference line
    - Color coding: Green (≥100% self-sufficient), Orange (<100% grid dependent)
    - Tooltip shows both net energy and self-sufficiency rate for each time point
    - **Frontend-only calculation** using timestamp-based matching (not database values)
    - **Critical Fix**: Database Net Energy values (TL339) were incorrect, now calculated from TL341 + TL340
    - Formula: Net Energy = Generation + Consumption (consumption is negative)
    - Self-Sufficiency Rate = (Generation / |Consumption|) × 100%
  - Explanatory info cards
  - Data source: TL341 (consumption), TL340 (generation) - TL339 no longer used due to data inaccuracy
...
- **Forecast Tab** (UPDATED 2026-01-05):
  ...
  - **Generation Forecast** (NEW):
    ...
    - Simple linear regression: `Generation = a × Direct_Radiation + b`
...
- **Performance Optimization**:
  ...
  - Optimized SQL queries using DATEPART for 270x faster performance (35s → 130ms)
...
- Frontend Components: 
  ...
  - Net Energy: `NetEnergyTab.jsx`, `NetEnergyMetricsCards.jsx`, `NetEnergyWithSelfSufficiencyChart.jsx`
...
- Utilities: `electricityCalculations.js` (Net Energy and Self-Sufficiency Rate calculations)
...
- Primary (634 days: 2019-02-13 to 2020-11-08):
  - TL341: Consumption Hourly Increment (used for forecast and self-sufficiency calculation)
  - TL340: Generation Hourly Increment (used for self-sufficiency calculation)
  - ~~TL339: Net Energy Hourly Increment~~ (DEPRECATED - data inaccurate, now calculated from TL341 + TL340)
...
- **Frontend-only Net Energy calculation** from consumption + generation (database values inaccurate)
- **Timestamp-based data matching** for accurate self-sufficiency rate calculation
...
### 6.1 Overview Dashboard (Fully Implemented - Enhanced 2026-01-07)

**Original Plan**: Not in original plan

**Actually Implemented**: Complete unified overview dashboard showing all modules on one page with enhanced component reusability
...
- **Enhanced Module Displays** (NEW - 2026-01-07):
  - **Electricity**: 
    - Two-column card layout (Consumption, Generation) + full-width Net Energy card
    - Reuses `MetricsCards`, `NetEnergyMetricsCards`, `OverallTrendChart`, `NetEnergyWithSelfSufficiencyChart`
    - Displays Self-Sufficiency Rate with dual Y-axis chart
    - Calculated Net Energy (not database values) for accuracy
  - **Water**: 
    - Two-column card layout (Rainwater, Hot Water)
    - Reuses `MetricsCards` and `OverallTrendChart` from Electricity module
    - Consistent styling with Electricity
  - **Thermal**: 
...
- **Compact Design**:
  - Reduced font sizes and spacing for overview context
...
- **Electricity**: 
  - Uses TL341 (Consumption) and TL340 (Generation)
  - Frontend calculates Net Energy and Self-Sufficiency Rate (timestamp-based matching)
  - Does NOT use TL339 (database Net Energy values are inaccurate)
...
- Consistent color scheme across all modules (SAIT colors)
- Interactive tooltips and legends
- Forced chart container heights to eliminate whitespace
...
- Frontend Components: 
  ...
  - `ElectricityOverview.jsx`, `WaterOverview.jsx`, `ThermalOverview.jsx` (module sections - ENHANCED)
  - `CollapsibleSection.jsx`, `NoDataMessage.jsx` (common components)
- Custom Hook: `useOverviewData.js` (data loading and aggregation - ENHANCED with Net Energy calculation)
- Utilities: 
  - `dataAggregation.js` (daily aggregation functions)
  - `electricityCalculations.js` (Net Energy and Self-Sufficiency Rate calculations)
- Constants: `overview.js` (date ranges, presets, labels)

**Technical Highlights**:

- **Component Reusability**: Directly reuses components from Electricity, Water, and Thermal report pages
  - `MetricsCards` - Used in Electricity and Water overviews
  - `NetEnergyMetricsCards` - Used in Electricity overview
  - `OverallTrendChart` - Used in Electricity and Water overviews
  - `NetEnergyWithSelfSufficiencyChart` - Used in Electricity overview
- **Consistent Styling**: All three modules use identical card layouts and sizing
- **Frontend Net Energy Calculation**: Uses `calculateNetEnergyFromData` for accurate results
- **Timestamp-based Matching**: Ensures data alignment across consumption, generation, and self-sufficiency
...
- CSS overrides for compact display (forced chart heights, reduced fonts)

**Implementation Dates**: 
- Initial Dashboard: 2026-01-04
- Enhanced with Component Reusability: 2026-01-07

---

### 6.2 Thermal Dashboard (Fully Implemented with Forecast and Outdoor Temperature)

**Original Plan**: P2 priority, skeleton implementation only

**Actually Implemented**: Complete functional thermal dashboard with forecast capability and outdoor temperature integration

**Features**:

- **Three View Modes**:
  - Single Day: Hourly data for selected date with outdoor temperature
  - Multiple Days: Aggregated data for date range with outdoor temperature
  - Forecast: Temperature prediction for next 7/14/30 days with outdoor temperature comparison
...
- **Data Visualization**:
  ...
  - Outdoor temperature overlay on all charts

- **Floor Plan Heat Map**:
  - Real-time temperature overlay on floor plan
  - Color-coded temperature zones (8 levels: < 0°C to > 35°C)
  - Multiple sensor support (3-5 sensors per floor)
  - **Outdoor temperature display** (bottom-left corner, updates every 15 minutes)

- **Time Control**:
  - Time slider (0-95 for 15-minute intervals in Single Day mode)
  - Multi-day slider in Multiple Days mode
  - Play/pause animation
  - Manual time selection

- **Thermal Forecast** (NEW - 2026-01-07):
  - **Hybrid Model**: 80% Historical Baseline + 20% Weather Adjustment
  - **Training**: 60 days of historical data
  - **Weather Integration**: Open-Meteo API for outdoor temperature forecast
  - **Multi-Sensor Aggregation**: Averages all sensors per floor
  - **Forecast Periods**: 7, 14, or 30 days
  - **Model Details**:
    - Historical baseline: Moving average of past 60 days
    - Weather coefficient: Learned from historical data (default 0.05 if < 0.02)
    - Formula: `Indoor Temp = Baseline × 0.8 + (Baseline + k × (Outdoor - Comfortable)) × 0.2`
  - **Visualization**:
    - Indoor temperature: Green solid line (comfortable/controlled)
    - Outdoor temperature: Red/Blue dashed line (hot/cold)
    - Summary statistics with decimal precision (1 decimal place)
  - **Info Card**: Displays model information, baseline, weather coefficient, training days

- **Outdoor Temperature Integration** (NEW - 2026-01-07):
  - **All View Modes**: Outdoor temperature displayed in all three modes
  - **Chart Display**:
    - Single Day: Horizontal line (daily average)
    - Multiple Days: Line with shaded area (High-Low range)
    - Forecast: Dashed line with shaded area
  - **Dynamic Colors**:
    - Red (#F44336): Outdoor warmer than indoor
    - Blue (#2196F3): Outdoor colder than indoor
  - **Floor Plan Display**:
    - Bottom-left corner box showing current outdoor temperature
    - Same color coding as indoor sensors
    - Updates synchronized with time slider (15-minute intervals)
  - **Tooltip Information**:
    - Shows Avg, High, Low for outdoor temperature
    - Removed Open/Close from indoor sensor tooltips
  - **Data Source**: Open-Meteo API (hourly data)

- **Temperature Color Coding** (Enhanced):
  - < 0°C: Black (freezing)
  - 0-20°C: Blue (cold)
...
- **Data Source**: SQL Server (using sqlcmdQuery.js) + Open-Meteo API

- **Export Feature**: PDF export capability for all views

**Key Implementation Files**:

- Frontend Pages: `ThermalPage.jsx`
- Frontend Components: 
  - Core: `ThermalControlPanel.jsx`, `ThermalChartSection.jsx`, `ThermalFloorPlan.jsx`, `ThermalTimeSlider.jsx`
  - Charts: `ThermalTrendChart.jsx`, `ThermalCandlestickChart.jsx`
  - Forecast: `ThermalForecastView.jsx`, `ThermalForecastInfo.jsx`
- Custom Hooks: `useThermalData.js`, `useFloorManagement.js`, `useTimeControl.js`
- Services: `ThermalService.js`, `WeatherService.js`
- Constants: `thermal.js` (temperature ranges, colors, view modes)
- Backend: 
  - Routes: `thermalRoutes.js`, `weatherRoutes.js`
  - Controllers: `thermalController.js`, `weatherController.js`
  - Services: `thermalService.js`, `forecastService.js`, `weatherService.js`

**Database Tables Used**:
- Thermal sensors: 13 sensors across 3 floors (20004-20016_TL2)
- Date range: 2019-01-01 to 2020-11-08
- Interval: 15 minutes (96 records per day)

**Technical Highlights**:
- Multi-sensor aggregation for floor-level temperature
- Hybrid forecast model combining historical patterns and weather data
- Hourly outdoor temperature mapped to 15-minute intervals
- Dynamic color coding based on indoor/outdoor temperature comparison
- Reuses ForecastChart component with customization
- All code follows enterprise standards (components < 150 lines)

**Why Implemented Early**: Served as pilot for SQL Server integration

**Implementation Dates**: 
- Initial Dashboard: 2025-12-20
- Forecast: 2026-01-07
- Outdoor Temperature Integration: 2026-01-07
...
**Actually Implemented**: Complete functional water dashboard with two tabs and comprehensive forecast capability

**Features**:

- **Two Main Tabs**:
  - Rainwater Harvesting: Water level monitoring (%) with forecast sub-tabs
  - Hot Water Consumption: Domestic hot water usage (L/h) with forecast sub-tabs

- **Rainwater Sub-tabs**:
  - Overview: Historical data visualization with metrics
  - Forecast: Water level prediction using weather-based linear regression (7/14/30 days)

- **Hot Water Sub-tabs**:
  - Overview: Historical data visualization with metrics
  - Forecast: Consumption prediction with multi-tier algorithm system (7/14/30 days)

- **Rainwater Forecast** (NEW - 2026-01-06):
  - **Weather-Based Linear Regression Model**:
    - Formula: `Daily Average Water Level = a × Precipitation + b`
    - Uses Open-Meteo API for precipitation forecast
    - Training: 60 days of historical data
    - Predicts daily average water level (not hourly)
    - Coefficient can be positive or negative (learned from data)
  - **Model Information Display**:
    - Shows prediction formula with dynamic coefficient explanation
    - Weather data source information (Open-Meteo API)
    - Training period details
    - No R² display (removed for simplicity)
  - **Forecast Configuration**:
    - Forecast periods: 7, 14, or 30 days
    - Auto-generates forecast when date or period changes
    - Info alert explaining forecast purpose
  - **Chart Display**:
    - Shows "Average" and "Range" instead of "Total" (percentage data)
    - Y-axis: "Daily Average Rainwater Level (%)"
    - Customized for percentage-based forecasts

- **Hot Water Forecast**:
  - Four-tier algorithm system (same as Electricity consumption):
    - Tier 1: Holt-Winters (Exponential Smoothing) - Best accuracy
    - Tier 2: Seasonal Decomposition - Good accuracy
    - Tier 3: Linear Regression - Moderate accuracy
    - Tier 4: Moving Average - Basic accuracy
  - Intelligent algorithm selection based on data quality
  - Training period: 1 year of historical data
  - Forecast periods: 7, 14, or 30 days
  - Data availability indicators and warnings
  - Reuses existing forecast components (DataAvailabilityCard, AlgorithmTiersGrid)d, ForecastChart, AlgorithmTiersGrid)
...
- **Data Visualization**:
  ...
  - Forecast chart with predicted values
  - Smart statistics display:
    - Energy data (Wh, L/h): Shows "Total" and "Avg/day"
    - Percentage data (%): Shows "Average" and "Range"
...
- **Performance Optimization**:
  ...
  - Auto-load data when switching tabs
  - Reuses existing forecast service (no code duplication)
...
- **UI Design**:
  ...
  - Sub-tab navigation for Hot Water (Overview/Forecast)

**Key Implementation Files**:

- Frontend Pages: `WaterReportPage.jsx`
- Frontend Components: 
  - Water: `RainwaterTab.jsx`, `HotWaterTab.jsx`, `RainwaterForecastView.jsx`, `HotWaterForecastView.jsx`
  - Shared: `TimeFilter.jsx` (refactored from Electricity)
  - Forecast: `DataAvailabilityCard.jsx`, `ForecastChart.jsx`, `AlgorithmTiersGrid.jsx`, `RainwaterForecastInfo.jsx`
- Custom Hooks: `useWaterData.js`
- Services: `WaterReportService.js`, `ForecastService.js` (frontend)
- Backend: `waterRoutes.js`, `waterController.js`, `waterService.js`, `forecastService.js`, `weatherService.js`
...
- Reused and enhanced existing components (OverallTrendChart, MetricsCards, ForecastChart)
...
- Smart chart statistics (Total vs Average/Range based on data type)
- Weather API integration for rainwater forecast
- Dynamic coefficient explanations (positive vs negative)

**Implementation Dates**: 
- Water Dashboard: 2026-01-04
- Hot Water Forecast: 2026-01-06
- Rainwater Forecast: 2026-01-06
...
| Forecast/Prediction Models  | MOSTLY DONE | Electricity (consumption + generation), Water (hot water + rainwater), and Thermal (hybrid model) forecasts implemented |
...
│   └── database.js      (Database configuration and constants)
│
...
- Frontend Services: 9 (added ForecastService, WeatherService)
- Backend Routes: 9 (added forecastRoutes, weatherRoutes)
- Backend Controllers: 8 (added forecastController, weatherController)
- Backend Services: 7 (added forecastService, weatherService)
- Custom Hooks: 10+ (added useForecastData, useThermalData enhancements)
...
- Electricity Components: 17 (7 shared + 3 consumption + 2 generation + 3 net energy + 2 forecast)
- Water Components: 5 (1 shared TimeFilter + 2 tabs + 2 forecast views)
- Thermal Components: 10 (5 core + 2 charts + 2 forecast + 1 floor plan)
- Forecast Components: 7 (DataAvailabilityCard, ForecastChart, AlgorithmTiersGrid, GenerationForecastInfo, RainwaterForecastInfo, ThermalForecastInfo, AvailableDataChecks)
...
2. **Advanced Dashboard Features**
   ...
   - ✅ Self-sufficiency rate visualization for Electricity (COMPLETED 2026-01-07)
...
- ✅ Migrate Electricity to SQL Server
- ✅ Migrate Water to SQL Server
- ✅ Migrate Carbon Footprint to SQL Server
- ✅ Integrate historical carbon intensity API
...
- Others treat it as UTC time: `2020-11-08 00:00:00 UTC` → converts to `2020-11-07 17:00:00 MST` (previous day!)
...
- Calgary is UTC-7, so worst case: `2020-11-08 12:00:00 UTC` → `2020-11-08 05:00:00 MST` (still same day)
...
4. **`utils/weatherConstants.js`** (Weather API Configuration)
   ...
   - Weather variables list (generation and rain types)
   - Training period configuration
   - Error messages
   - Supports both generation forecast (solar radiation) and rainwater forecast (precipitation)
...
- Weather variables: 
  - For generation forecast: Direct radiation, temperature, cloud cover, shortwave radiation, diffuse radiation
  - For rainwater forecast: Precipitation, rain, showers, weather code
...
**Methods**:
- `getHistoricalWeather(dateFrom, dateTo, type)`: Fetch historical weather (type: 'generation' or 'rain')
- `getForecastWeather(dateFrom, dateTo, type)`: Fetch weather forecast (type: 'generation' or 'rain')
- `getWeatherData(dateFrom, dateTo, type)`: Auto-select historical or forecast
...
**New Methods for Rainwater Forecast** (2026-01-06):
- `generateRainwaterForecast()`: Main entry point for rainwater level prediction
- `trainRainwaterModel()`: Train simple linear regression model for rainwater
- `predictWithRainwaterModel()`: Generate predictions using trained model
