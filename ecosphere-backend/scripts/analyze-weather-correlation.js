// Weather Correlation Analysis Script
...
function aggregateToDaily(hourlyData, aggregationType = 'sum') {
    ...
    hourlyData.forEach(item => {
        const date = item.ts.split(' ')[0]; // Extract date part
...
function aggregateWeatherToDaily(weatherData) {
    ...
    // Calculate daily averages and sums
    const result = {};
    Object.keys(dailyWeather).forEach(date => {
        ...
        result[date] = {
            ...
            avg_diffuse_radiation: data.diffuse_radiation.reduce((a, b) => a + b, 0) / data.diffuse_radiation.length,
...
async function analyzeCorrelation() {
    console.log('🔍 Starting Weather-Generation Correlation Analysis\n');
    console.log('='.repeat(70));

    // Date range for analysis - use last 60 days before 2020-11-08
    // This gives us more data for better correlation analysis
    const dateTo = '2020-11-07'; // Last complete day with data
    const dateFrom = '2020-09-08'; // 60 days before
    ...
    try {
        ...
        console.log('\n' + '='.repeat(70));
        console.log('📊 DAILY GENERATION AND WEATHER DATA');
        console.log('='.repeat(70));

        dates.forEach(date => {
            const gen = dailyGeneration[date];
            const weather = dailyWeather[date];
            console.log(`\n${date}:`);
            console.log(`  Generation: ${gen.toFixed(2)} Wh`);
            console.log(`  Avg Temperature: ${weather.avg_temperature.toFixed(1)}°C`);
            console.log(`  Avg Cloud Cover: ${weather.avg_cloud_cover.toFixed(1)}%`);
            console.log(`  Total Shortwave Radiation: ${weather.total_shortwave_radiation.toFixed(0)} W/m²`);
            console.log(`  Avg Shortwave Radiation: ${weather.avg_shortwave_radiation.toFixed(1)} W/m²`);
            console.log(`  Total Direct Radiation: ${weather.total_direct_radiation.toFixed(0)} W/m²`);
            console.log(`  Total Diffuse Radiation: ${weather.total_diffuse_radiation.toFixed(0)} W/m²`);
            console.log(`  Daylight Hours: ${weather.daylight_hours}`);
            console.log(`  Avg Wind Speed: ${weather.avg_wind_speed.toFixed(1)} km/h`);
            console.log(`  Avg Humidity: ${weather.avg_humidity.toFixed(1)}%`);
        });
