// Check data interval for Solar Source tables
...
async function checkSolarInterval() {
  const tables = [
    { name: 'TL252', desc: 'Carport Solar' },
    { name: 'TL253', desc: 'Rooftop Solar' }
  ];
  ...
  console.log('Solar Source Tables Data Interval Check:');
  console.log('='.repeat(70));
  
  for (const table of tables) {
    // Count records
    const countQuery = `SELECT COUNT(*) as cnt FROM [SaitSolarLab_30000_${table.name}] WHERE CONVERT(varchar, ts, 23) >= '2020-11-01' AND CONVERT(varchar, ts, 23) <= '2020-11-08'`;
    const countCmd = `sqlcmd -S ${DB_SERVER} ${authParams} -d ${DB_DATABASE} -Q "${countQuery}" -s "," -W -h -1`;
    
    // Get first 5 timestamps
    const tsQuery = `SELECT TOP 5 CONVERT(varchar, ts, 120) as ts FROM [SaitSolarLab_30000_${table.name}] WHERE CONVERT(varchar, ts, 23) >= '2020-11-01' ORDER BY ts`;
