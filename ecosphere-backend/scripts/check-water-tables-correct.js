// Script to check CORRECT water-related tables
...
// CORRECT Water-related tables
const WATER_TABLES = [
    { name: 'SaitSolarLab_30000_TL86', desc: '1ETH058A - Humidity Voltage Output-TL' },
    { name: 'SaitSolarLab_30000_TL30', desc: 'N1ETH058A - Humidity-TL' },
    { name: 'SaitSolarLab_30000_TL29', desc: 'S1ETH063 - Humidity-TL' },
    { name: 'SaitSolarLab_30000_TL210', desc: 'GBT Domestic Hot Water consumption ⭐' },
    { name: 'SaitSolarLab_20000_TL93', desc: 'Rain_Water_Level_POLL ⭐' }
];
...
async function analyzeTable(tableInfo) {
    ...
    console.log(`Description: ${tableInfo.desc}`);
    ...
    try {
        ...
        const infoLines = infoOutput.trim().split('\n').filter(line => line.trim() && !line.includes('rows affected') && !line.includes('????'));
        ...
        const negQuery = `SELECT COUNT(*) as negative_count FROM [${tableName}] WHERE value < 0`;
        const negCommand = buildCommand(negQuery);
        const { stdout: negOutput } = await execPromise(negCommand);
        const negLines = negOutput.trim().split('\n').filter(line => line.trim() && !line.includes('rows affected') && !line.includes('????'));
        console.log('Count of negative values:');
        negLines.forEach(line => console.log(line));

        // 7. Value distribution (sample ranges)
        console.log('\n7. VALUE DISTRIBUTION (Sample):');
        const distQuery = `SELECT TOP 20 CONVERT(varchar, ts, 120) as timestamp, value FROM [${tableName}] ORDER BY value DESC`;
        const distCommand = buildCommand(distQuery);
        const { stdout: distOutput } = await execPromise(distCommand);
        const distLines = distOutput.trim().split('\n').filter(line => line.trim() && !line.includes('rows affected') && !line.includes('????'));
        console.log('Top 20 highest values (Timestamp, Value):');
        distLines.slice(0, 5).forEach(line => console.log(line));
        console.log('...');
...
async function main() {
    console.log('CORRECT WATER-RELATED TABLES ANALYSIS');
    ...
    for (const tableInfo of WATER_TABLES) {
    ...
    console.log('='.repeat(80));
    console.log('\nSUMMARY:');
    console.log('- Humidity tables (TL86, TL30, TL29): Environmental data');
    console.log('- TL210: Domestic Hot Water consumption (PRIMARY for dashboard)');
    console.log('- TL93: Rain Water Level (SECONDARY - rainwater harvesting)');
}
