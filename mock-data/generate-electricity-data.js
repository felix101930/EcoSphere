...
console.log(`📊 Total records: ${electricityData.length}`);
console.log(`📅 Date range: ${electricityData[0].ts} to ${electricityData[electricityData.length - 1].ts}`);
console.log(`⚡ Power range: ${Math.min(...electricityData.map(d => d.value)).toFixed(2)} W to ${Math.max(...electricityData.map(d => d.value)).toFixed(2)} W`);
